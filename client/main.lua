...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.CornProcessing.coords, true) < 3 then
			DrawMarker(27, Config.CircleZones.CornProcessing.coords.x, Config.CircleZones.CornProcessing.coords.y,
			 Config.CircleZones.CornProcessing.coords.z - 1 , 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 255, 0, 0, 200, 0, 0, 0, 0)

			if not isProcessing then
				Draw3DText(Config.CircleZones.CornProcessing.coords.x, Config.CircleZones.CornProcessing.coords.y, Config.CircleZones.CornProcessing.coords.z, 
				 '[E] - Start Processing Corn', 4, 0.08, 0.08, Config.SecondaryColor)
			end

			if IsControlJustReleased(0, 38) and not isProcessing then
				TriggerServerEvent('qb-farming:ProcessCorn')
			end
		...
		elseif GetDistanceBetweenCoords(coords, Config.CircleZones.OrangePack.coords, true) < 3 then
			...
			if not isProcessing then
				Draw3DText(nowcoords.x, nowcoords.y,nowcoords.z, 
				'[E] - Pack Oranges', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			if IsControlJustReleased(0, 38) and not isProcessing then
				TriggerServerEvent('qb-farming:ProcessOranges')
			end
		elseif GetDistanceBetweenCoords(coords, Config.CircleZones.MilkPack.coords, true) < 3 then
			...
			if not isProcessing then
				Draw3DText(nowcoords.x, nowcoords.y,nowcoords.z, 
				'[E] - Prepare Milk Pack', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			if IsControlJustReleased(0, 38) and not isProcessing then
				TriggerServerEvent('qb-farming:ProcessMilk')
			end

		elseif GetDistanceBetweenCoords(coords, Config.TractorCoords, true) < 3 then
			if not rented then
				Draw3DText(Config.TractorCoords.x, Config.TractorCoords.y, Config.TractorCoords.z,
				 '[E] - Rent a Tractor for Farming', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			if rented then 
				Draw3DText(Config.TractorCoords.x, Config.TractorCoords.y, Config.TractorCoords.z,
				 '[E] - Return Tractor', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			...
			if IsControlJustReleased(0, 38) and not rented then
				...
				TriggerServerEvent('qb-farming:server:SpawnTractor')
			end
			if IsControlJustReleased(0, 38) and rented then
				...
				TriggerServerEvent('qb-farming:server:UnRentTractor')
			end
		else
			Citizen.Wait(500)
		end
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						if IsControlJustReleased(0, 38) then
                            ...
                            QBCore.Functions.Progressbar("sell_pawn_items", "Selling stuff ..", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
								...
								TriggerServerEvent("qb-farming:server:SellFarmingItems")
								sellItemsSet = false
								sellPrice = 0
                            end, function() -- Cancel
								ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('qb-farming:SpawnTractor')
AddEventHandler('qb-farming:SpawnTractor', function()
...
RegisterNetEvent('qb-farming:UnRentTractor')
AddEventHandler('qb-farming:UnRentTractor', function(rentveh)
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				...
				Draw3DText(coord1.x, coord1.y, coord1.z + 1.5, 
				'[E] - Pick Up Corn Kernel', 4, 0.08, 0.08, Config.SecondaryColor)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				QBCore.Functions.Progressbar("search_register", "Picking up Corn Kernel!", 2500, false, true, {
					...
					TriggerServerEvent('qb-farming:pickedUpCannabis')
					-- TriggerServerEvent('qb-farming:weed')
		...
		elseif nearbySpot and GetEntityModel(playerVehicle) == `tractor3` then
			...
			if not isPickingUp then
				...
				Draw3DText(coord.x, coord.y, coord.z + 1.5, 
				'[E] - Mow the Field', 4, 0.2, 0.2, Config.SecondaryColor)
			end
	...
	while true do
		...
		if nearbycow and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				...
				Draw3DText(coord1.x, coord1.y, coord1.z + 1.5, 
				'[E] - Milk The Cow', 4, 0.07, 0.07, Config.SecondaryColor)
			end
...
RegisterNetEvent('qb-farming:internal:CowProgress')
AddEventHandler('qb-farming:internal:CowProgress', function()
	print(prog)
	while prog < 101 do 
		print(prog)
		prog = prog + 1
...
function MilkCow(nearbycow)
	TriggerEvent('qb-farming:internal:CowProgress')
	isPickingUp = true	
	while true do 
		Citizen.Wait(5)
		...
		if prog == 100 then 
			TriggerServerEvent('qb-farming:CowMilked')
			isPickingUp = false
			ClearPedTasks(PlayerPedId())
...
function GetSellingPrice()
	local price = 0
	QBCore.Functions.TriggerCallback('qb-farming:server:GetSellingPrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
	...
	QBCore.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CornProcessing.coords, false) > 4 then
				TriggerServerEvent('qb-farming:cancelProcessing')
				break
			end
		end
		ClearPedTasks(PlayerPedId())
...
function PickOrange()
	...
	QBCore.Functions.Progressbar("search_register", "Picking Up oranges!", 15000, false, true, {
		...
		TriggerServerEvent('qb-farming:GiveOranges')
		ClearPedTasks(PlayerPedId())
		Citizen.Wait(5000)
		oranges = false
	end, function()
